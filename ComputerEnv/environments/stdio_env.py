...
class StdIOEnv(BaseComputerEnv):
    ...
    def __init__(self, keys=ASCII_128_KEYS,
                 columns=80, rows=24, buffer_size=4096,
                 command='bash', cwd=None, env=None):

        # keyboard
        self.keyboard = Keyboard(
            press_key_fn=lambda key: None,
            release_key_fn=lambda key: None,
            keys=keys
        )
        ...
        self.env.update(dict(
            TERM="linux", COLUMNS=self.terminal_display.columns,
    ...
    def step(self, action):
        ...
        # type to stdin
        keys_down = [
            key for idx, key in enumerate(self.keyboard.keys)
    ...
    def render(self, mode='human'):
        if mode == 'human':
            ...
            raise NotImplementedError(
                f'render mode `{mode}` not supported')
