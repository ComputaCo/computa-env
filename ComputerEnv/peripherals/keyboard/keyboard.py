...
class Keyboard(Peripheral):
    ...
    def __init__(self, press_key_fn, release_key_fn,
                 is_pressed_fn=None, keys=None, name='keyboard'):
        ...
        self.observation_space = gym.spaces.MultiBinary(len(self.keys))
        self.action_space = gym.spaces.MultiBinary(len(self.keys))
        self.reset()
    ...
    def get_observation(self):
        if self.is_pressed_fn:
            ...
            self._key_state = np.array([self.is_pressed_fn(key)
                                        for key in self.keys], dtype=int)
