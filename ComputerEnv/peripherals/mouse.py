...
class Mouse(Peripheral):
    ...
    def __init__(self,
                 press_button_fn, release_button_fn, wheel_fn,
                 move_fn, position_fn, is_pressed_fn,
        ...
        self.observation_space = gym.spaces.Dict({
            ...
            'buttons': gym.spaces.MultiBinary(len(self.buttons))
        })
        self.action_space = gym.spaces.Dict({
            'motion': gym.spaces.Box(low=-np.inf, high=np.inf, shape=(2,)),
            'wheel_delta': gym.spaces.Box(low=-np.inf, high=np.inf, shape=(1,)),
            'buttons': gym.spaces.MultiBinary(len(self.buttons))
        })
    ...
    def get_observation(self):
        return {
            ...
            'buttons': np.array([self.is_pressed_fn(button) for button in self.buttons], dtype=int)
        }
